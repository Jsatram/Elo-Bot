...
matchesdb = sqlite3.connect('newmatches.db')
tournamentdb = sqlite3.connect('tournament.db')
# newmatchesdb = sqlite3.connect('newmatches.db')

pc = playersdb.cursor()
mc = matchesdb.cursor()
tc = tournamentdb.cursor()
# nmc = newmatchesdb.cursor()
...
async def printteams(ctx):
  ...
  for each in matches:
    team1 = getteamid(each[2])
    team2 = getteamid(each[3])
    print(team1)
    print(team2)
...
async def dbtransfer(ctx):
  ...
  for each in matches:
    ...
    if count >= 28:
      ...
      nteam1 = ""
      for player in team1:
        pc.execute("SELECT * FROM players WHERE userid=?",(int(player),))
        user = pc.fetchone()

        
        nteam1 = nteam1 + str(user[1]) + ","
        nteam1 = nteam1 + str(user[6]) + ","
        nteam1 = nteam1 + str(user[7]) + ","
        nteam1 = nteam1 + str(user[8]) + "‰"
      ...
      for player in team2:
        ...
        nteam2 = nteam2 + str(user[1]) + ","
        nteam2 = nteam2 + str(user[6]) + ","
        nteam2 = nteam2 + str(user[7]) + ","
        nteam2 = nteam2 + str(user[8]) + "‰"
      ...
      nmc.execute("INSERT INTO matches VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",(each[0] , each[1],nteam1,nteam2,each[4],each[5] ,each[6] ,each[7] ,each[8],each[9] ,each[10],each[11],each[12]))
      newmatchesdb.commit()
...
async def EUregister(ctx):
  ...
  if user != None:
    await directmsg(ctx.author.id,"Seems you are already registered. *please contact an AoV admin*")
    print("Found!")
...
@client.command()
@commands.has_role("AoV")
async def setstreak(ctx,*args):
  try:
    pc.execute("UPDATE players SET winstreak = ? WHERE userid= ?",(int(args[1]),int(args[0]),))
    playersdb.commit()
...
async def resolve(ctx, *args):
  ...
  try:
    ...
    if(matchid <= len(temp)+27):
      ...
      team1 = getteamid(match[2])
      team2 = getteamid(match[3])
...
@client.command()
@commands.has_role("AoV")
async def printinfo(ctx, *args):
  matchid = int(args[0].strip())
  nmc.execute("SELECT * FROM matches WHERE matchid=?",(matchid,))
  match = nmc.fetchone()

  playerinfo = []
  for info in getteaminfo(match[2]):
    playerinfo.append(list(info.split(",")))
    
  print(playerinfo)

@client.command()
@commands.has_role("AoV")
async def unresolve(ctx, *args):
  ...
  try:
    ...
    if match[12] == 1 and match[8] == 0:
      mc.execute("UPDATE matches SET final = ? WHERE matchid= ?",(0,matchid,))
      matchesdb.commit()

      if match[10] == 1:

        for each in getteamelo(match[2]):
          rank1 = rank1 + int(each)

        for each in getteamelo(match[3]):
          rank2 = rank2 + int(each)
        ...
        team1info = []
        for info in getteaminfo(match[2]):
          team1info.append(list(info.split(",")))

        team2info = []
        for info in getteaminfo(match[3]):
          team2info.append(list(info.split(",")))

        for each in team1info:
          pc.execute("SELECT * FROM players WHERE userid=?",(int(each[0]),))
          user = pc.fetchone()
          user=list(user)
          ...
          bonus = getbonuswithstreak(int(each[2]), 1)

          print(user[1])
          print("Bonus: " + str(bonus))
          print("t1: " + str(t1))
          if inplacements == 1:
            pc.execute("UPDATE players SET wins = ? WHERE userid= ?",(int(user[4]-1),int(each[0]),))
            pc.execute("UPDATE players SET elo = ? WHERE userid= ?",(int(user[6]-35-bonus),int(each[0]),))
            playersdb.commit()
            
          else:
            pc.execute("UPDATE players SET wins = ? WHERE userid= ?",(int(user[4]-1),int(each[0]),))
            pc.execute("UPDATE players SET elo = ? WHERE userid= ?",(int(user[6]-t1-bonus),int(each[0]),))
            playersdb.commit()

        for each in team2info:
          pc.execute("SELECT * FROM players WHERE userid=?",(int(each[0]),))
          user = pc.fetchone()
          user=list(user)
          ...
          bonus = getbonuswithstreak(int(each[2]), 0)

          print(user[1])
          print("Bonus: " + str(bonus))
          print("t2: " + str(t2))
          if inplacements == 1:
            pc.execute("UPDATE players SET losses = ? WHERE userid= ?",(int(user[5]-1),int(each[0]),))
            pc.execute("UPDATE players SET elo = ? WHERE userid= ?",(int(user[6]+35-bonus),int(each[0]),))
            playersdb.commit()
            
          else:
            pc.execute("UPDATE players SET losses = ? WHERE userid= ?",(int(user[5]-1),int(each[0]),))
            pc.execute("UPDATE players SET elo = ? WHERE userid= ?",(int(user[6]-t2-bonus),int(each[0]),))
            playersdb.commit()
      ...
      channel = client.get_channel(eloMod)
      await channel.send("Match does not seem to be final yet.")
...
async def makeMatch(listp):
  ...
  team1 = list(team1.split("‰"))
  team2 = list(team2.split("‰"))
  ...
  mc.execute("INSERT INTO matches VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",(matchid , str(current_time), nteam1, nteam2 , hp, snd, ctrl, False,False,0 ,0,"",False))
  matchesdb.commit()
...
def checkFinal(matchid):
  ...
  print("count t1 " + str(t1count))
  print("count t2 " + str(t2count))
...
def getteamstreak(team):
  
  userinfo = list(each.split("‰"))
  
  streaks = []
  for each in userinfo:
    temp = list(each.split(","))
    streaks.append(temp[2])

  return streaks

def getteamplacements(team):
  ...
  for each in userinfo:
    temp = list(each.split(","))
    placements.append(temp[2])

  return placements

def getbonuswithstreak(streak, win):
  bonus = 0

  if streak > 0 and win == 1:
    streak = streak + 1

  elif streak < 0 and win == 1:
    streak = 1

  elif streak < 0 and win == 0:
    streak = streak - 1
